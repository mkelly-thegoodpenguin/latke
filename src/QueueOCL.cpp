...
QueueOCL::QueueOCL(DeviceOCL *device, cl_command_queue_properties queue_props) :
...
#endif
	if (!queue) {
		queue = clCreateCommandQueue(device->context, device->device, queue_props, &errorCode);
		if (errorCode != CL_SUCCESS)
